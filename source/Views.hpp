...
public:
	...
	EXIT_CODE moveSprite(sf::Vector2f vel, sf::Sprite& s) {
		s.move(vel);
		return EXIT_CODE::OPERATION_SUCCESFUL;
	}
	...
	/// <returns></returns>
	EXIT_CODE moveViewAndSprite(sf::Vector2f vel, sf::Sprite& sprt, bool& cj, bool& ij) {
		// Update the view position
		updateView();
		moveView(vel);
		setSpriteInMiddleOfView(sprt);

		while (Ground::collision_with_ground(sprt) == EXIT_CODE::SPRITE_COLLIDED) {
			ij = false;
			cj = true;
			moveView(sf::Vector2f(0, -0.5f));
			setSpriteInMiddleOfView(sprt);
			}
	...
	EXIT_CODE setViewInMiddleOfSprite(sf::Sprite& sprt) {
